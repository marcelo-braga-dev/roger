...
export default function ({id, dados}) {
    ...
    function submit(e) {
        ...
        if (data.senha && data.senha === data.confirmar) {
            router.put(route('admin.usuarios.vendedores-senhas-update', id), {
                '_method': 'put',
                ...data
            })
            return
    ...
    return (
        <Layout container titlePage="Atualizar Senha" menu="usuarios" voltar={route('admin.usuarios.vendedores.show', id)}>
            <div className="mb-5">
                <DadosUsuario dados={dados}/>
            </div>

            {alerta && <div className="text-danger mb-3">Senha não concidem.</div>}
